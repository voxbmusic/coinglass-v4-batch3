...
Bartu Bartucan, bu iş için Research’ü açman en yararlı olanı; çünkü senin istediğin şey “CoinGlass + Reddit + GitHub + diğer kaynaklar” üzerinden endpoint/limit/workaround/mimari taraması ve çapraz doğrulama. “Search” daha hızlı ama daha yüzeysel kalır; “Agent” ise araştırmadan çok iş yaptırma/otomasyon tarafında (komut çalıştırma, dosya üretme, rutin işler) anlamlı. Sen şu an “tam kapsam haritalama + mimari plan” istediğin için Research doğru seçim. Bu pencerede konuşulan her şeyi “kelimesi kelimesine” kalıcı hafızaya yazmayı teknik olarak garanti edemiyorum; ama projeyi ilerletmek için gereken bütün kritik noktaları (root-cause, fix’ler, repo durumu, komutlar, mimari kararlar, plan kilitleri, hangi endpoint nerede) tek bir “Project Log” metni olarak burada derli toplu çıkaracağım—istersen bunu repo içine PROJECT_LOG.md olarak da koyarız ki kayıp riski sıfırlansın. Şimdi gelelim asıl konuya: “Savaş Paneli”nin geri kalanını %100’e yakın terminalden çekmek için Kaynak Haritası + Mimari Plan.

CoinGlass tarafında iyi haber: CoinGlass v4 API referansında (open-api-v4.coinglass.com) senin listendeki çok büyük bir kısmı kapsayan kategoriler var: Futures (Liquidation Map/Heatmap Model 1/2/3, Liquidation Order, Orderbook Heatmap, Large Open Orders, CVD, Netflow), Spots (market/ohlc/orderbook/footprint/cvd/netflow), ETF (Flows/NetAssets/Trading/Price/Detail/AUM, Premium/Discount, Grayscale holdings & premium), On-Chain (Exchange balance list/chart, whale transfer, exchange transfers), Indic/Other (AHR999, Puell, Stock-to-Flow, Pi Cycle, Golden Ratio Multiplier, Bubble Index, 2-Year MA Multiplier, Fear & Greed, ayrıca BTC STH/LTH Supply, STH/LTH SOPR, RHODL, Reserve Risk, NUPL vb.). Bu menü, “LTH Supply var mı?” sorusuna da net cevap veriyor: var ve API altında listelenmiş.  

Buna göre, bugün terminalde çalışan Daily Minimal 9/10 zaten “Savaş Paneli”nin yalnızca %20’si değil; senin bütün “51 metriklik” tam listenin içinde küçük bir alt küme. Ancak “btc komutu bugün sadece Daily Minimal basıyor” diye toplam kapsam %20 sanıyorsun. Doğru okuma şu: altyapı artık stabil; bundan sonra registry’yi büyütmek + doğru endpoint’leri eklemek + normalizer’ları yazmak işi kalıyor. En önemli hızlanma burada olacak.

Aşağıdaki “Kaynak Haritası” planını CoinGlass v4 dokümanındaki modüllere göre çıkarıyorum; yanında “CoinGlass ile olur mu?” ve “gerekirse ücretsiz fallback” notunu da ekliyorum:

DAILY – Türev & Likidite
1 Real-time Open Interest → CoinGlass Futures OI endpoints (zaten var)  
2 OI Change ($, %) → OI history/ohlc üzerinden türet (zaten var)  
3 Weighted Funding Rate → Futures Funding Rate “OI Weight / Vol Weight / History” (zaten var)  
4 Long/Short Ratio (Global) → Futures Long-Short “Global Account Ratio” (daily_06 çözüldü)  
4b Long/Short (Hyperliquid) → CoinGlass’ta Hyperliquid Positions endpoints var (Startup plan izin verirse çekilir; vermezse Hyperliquid’in kendi public verisiyle ücretsiz adapter yazılır)  
5 Liquidation Map → “Pair/Coin Liquidation Map” endpoints var  
6 Liquidation Heatmap (Model2/3) → “Pair Liquidation Heatmap Model2/3” endpoints var  
7 Top Liquidation Events → CoinGlass “Liquidation Order” (websocket + REST) veya “Pair Liquidation History” + event extraction; sen şu an aggregated-history’den “top” gibi kullanıyorsun ama gerçek “event” için Liquidation Order daha doğru  
8 Orderbook Liquidity Delta → Futures Orderbook endpoints + “Large Open Orders History” + net değişim hesabı; ayrıca Orderbook Heatmap endpoint var  
9 Perpetual / Spot Volume → Futures & Spot volume history + ayrıca “Futures vs Spot Volume Ratio” endpoint CoinGlass’ta var  

DAILY – Spot & Kurumsal Akış
10 Coinbase Bitcoin Premium Index → zaten çekiyorsun (Indic → Spots)  
11 Premium Index → CoinGlass’ta “Premium Index” ayrı endpoint olarak dokümanda görünüyor (Indic/Spots altında Coinbase premium var; genel premium için ilgili endpoint’i registry’ye ekleyeceğiz)  
12 Spot BTC ETF Flows (USD) → ETF “ETF Flows History”  
13 Spot BTC ETF Net Flows → Flows history’den net türet veya ETF History/Detail’den al (dokümanda ETF History/Detail var)  
14 Spot BTC ETF Trading Volume → ETF “ETF History” veya “ETF Price History” / “ETF Detail” içinde (dokümanda var)  

DAILY – Momentum & Aşırılık
15 RSI → Indic/Futures “Pair RSI” veya “Coin RSI List”  
16 Funding Rate History → zaten çekiyorsun (Funding Rate History OHLC)  
17 Volume (24h) → Spot/Futures market endpoints + OHLC/market summary; 24h volume genelde market endpoint’te olur  
18 Volume Change % → (17)’den türet

WEEKLY – Smart Money & Yapı
Kurumsal
1 CME Long/Short Report → CoinGlass’ta “COT/CME report” doğrudan menüde görünmüyor; bunu ücretsiz kaynakla çözmek daha gerçekçi (CFTC COT raporu). CoinGlass varsa da ayrıca araştırıp endpoint çıkarırız. (Burada Research taraması gerekli.)
2 CME Open Interest → CoinGlass Futures OI exchange breakdown ile yaklaşılabilir; CME özel OI yoksa fallback: CME verisi sunan ücretsiz kaynaklar.
3 MicroStrategy Avg BTC Cost → CoinGlass menüsünde yok; bunu ücretsiz hesaplarız (MSTR BTC holdings + toplam maliyet bilgisi).
4 ETFs Premium/Discount → CoinGlass ETF “Premium/Discount History”  
5 Grayscale Holdings → CoinGlass “Holdings List”  
6 Grayscale Premium → CoinGlass “Premium History”  

Borsa & Cüzdan
7 Bitcoin Exchange Balance → CoinGlass On-Chain “Exchange Balance List/Chart”  
8 Wallet Inflow/Outflow → CoinGlass’ta “Netflow” (Coin NetFlow) var; spot/futures için ayrı ayrı var  
9 Whale Revived Supply → CoinGlass On-Chain’de “Whale Transfer” var ama “revived supply” spesifik metriği doğrudan görünmüyor; gerekirse ücretsiz on-chain kaynakla tamamlanır.  
10 Miner Outflows → CoinGlass menüsünde net görünmüyor; muhtemelen dış kaynak gerekir.
11 Miner Revenue → CoinGlass’ta net görünmüyor; dış kaynak gerekir.

Holder
12 LTH Supply → CoinGlass Indic/Other “Bitcoin Long Term Holder Supply”  
13 STH Supply → “Bitcoin Short Term Holder Supply”  
14 LTH SOPR → “Bitcoin Long Term Holder SOPR”  
15 STH SOPR → “Bitcoin Short Term Holder SOPR”  
16 LTH-MVRV → CoinGlass menüsünde “MVRV” açıkça görünmüyor; belki farklı isimle var (Research ile netleştireceğim). Yoksa ücretsiz fallback gerekir.
17 STH-MVRV → aynı durum
18 RHODL Ratio → CoinGlass “Bitcoin RHODL Ratio”  

MONTHLY – Cycle & Değerleme
1 MVRV Ratio → menüde açık isimle yok; Research ile “başka isimle var mı” bakılacak, yoksa ücretsiz fallback
2 MVRV Z-Score → aynı
3 Mayer Multiple → menüde açık isimle yok; fallback muhtemel
4 Puell Multiple → CoinGlass “Puell-Multiple”  
5 NVT Ratio → menüde açık isimle yok; fallback muhtemel
6 Bitcoin Bubble Index → CoinGlass “Bitcoin Bubble Index”  
7 Ahr999 → CoinGlass “AHR999”  
8 2-Year MA Multiplier → CoinGlass “Tow Year Ma Multiplier”  
Döngü/Model
9 4-Year MA → menüde açık isimle yok; fallback
10 Stock-to-Flow → CoinGlass “Stock-to-Flow Model”  
11 Pi Cycle Top → CoinGlass “Pi Cycle Top Indicator”  
12 Golden Ratio Multiplier → CoinGlass “Golden-Ratio-Multiplier”  
13 Hash Ribbons → menüde açık isimle yok; fallback
14 Bitcoin Power Law → menüde açık isimle yok; fallback
15 Delta Top Model → menüde açık isimle yok; fallback

Bu tablo bize şunu söylüyor: CoinGlass v4 API, senin listenin çok büyük kısmını kapsıyor; ama birkaç kritik metrikte (CME raporları, miner metrikleri, MVRV/NVT/Mayer/Hash Ribbons/Power Law/Delta Top gibi) CoinGlass’ın API menüsünde doğrudan isim görmeyince “ya farklı isim” ya da “dış kaynak” ihtimali doğuyor. Burada Research ile tek tek doğrulayıp “CoinGlass’ta endpoint var mı/yok mu”yu kesinleştiririz. Yine de şimdiden makul bir mühendislik tahmini: CoinGlass ile 51 metriğin ~35–45’i (yaklaşık %70–%88 bandı) doğrudan çekilebilir; kalan ~6–16 metrik (özellikle “CME/madenci/MVRV-NVT/Mayer/HashRibbons/PowerLaw/DeltaTop/RevivedSupply”) için ücretsiz adaptör gerekir. Bu tahminin sebebi CoinGlass doküman menüsünde zaten LTH/STH supply+SOPR+RHODL+ReserveRisk+NUPL gibi “holder” ailesinin bile bulunması; bu, kapsamın sandığından çok daha geniş olduğunu gösteriyor.  

MİMARİ PLAN (coinglass-v4-batch3 üzerinde)
Hedef: btc komutu tek çalıştırmada “Daily / Weekly / Monthly” tamamını basacak; her metrik her zaman görünecek (OK / LOCKED / EXTERNAL_REQUIRED / MISSING). JSON + Text birlikte üretilecek.
	1.	Registry genişletme stratejisi

	•	batch3_metrics_system/metric_registry.py içinde yeni üç liste: daily_full, weekly, monthly (veya tek registry içinde category+cadence alanı).
	•	Her metrik için: id, name, timeframe, cadence (daily/weekly/monthly), source (“coinglass_v4”, “fallback_x”), endpoint, params, normalizer, status_hint (plan-locked olasılığı).
	•	Daily Minimal (10) aynen kalsın; Daily Full (18) bunun üstüne eklensin.

	2.	Adapter katmanı

	•	Bugünkü CoinGlass adapter’ı: batch2_engine/coinglass.py ve CoinGlassAPI.fetch() iyi gidiyor. Bunun üstüne “source adapter” soyutlaması ekleyeceğiz:
	•	adapters/coinglass_v4.py (REST + opsiyonel websocket)
	•	adapters/hyperliquid_public.py (Hyperliquid Long/Short/positions, eğer CoinGlass plan kilitliyse)
	•	adapters/cot_cftc.py (CME long/short report için CFTC COT)
	•	adapters/miner_metrics.py (miner outflows/revenue için ücretsiz kaynak)
	•	adapters/onchain_fallback.py (revived supply vb. gerekiyorsa)

	3.	Normalizer katmanı

	•	batch3_metrics_system/normalizer.py dosyasında her endpoint ailesi için normalize fonksiyonları. Senin yaptığın “unwrap + liquidation totals/events” patch’i doğru yönde; bunu “çekirdek normalizer seti” haline getirip her yeni metrik için normalize ekleyeceğiz.
	•	Normalizer standardı: her metrik JSON Contract v1’e uygun tek tip output döndürsün (ör. {value, unit, meta, ts} ya da list of items).

	4.	Caching / rate limit / retry

	•	CoinGlass Startup plan rate-limit’e takılmamak için TTL cache şart:
	•	Daily: 60–180 saniye cache (OI, funding, premium)
	•	Heatmap/Map/Orderbook: 60–120 saniye (ağır endpointler)
	•	Weekly: 6–24 saat
	•	Monthly: 1–7 gün
	•	Disk cache (json) + memory cache hibrit yapılır.

	5.	CLI orchestration

	•	btc_cli.py artık “Daily Minimal” değil “Savaş Paneli (Daily/Weekly/Monthly)” basacak.
	•	Bayraklar: btc --daily-min, btc --daily, btc --weekly, btc --monthly, btc --json, btc --text (varsayılan ikisi birden).
	•	“Always show all metrics” kuralı: başarısız olsa bile satır bas (status=MISSING/LOCKED vs).

ŞİMDİ “HIZLI İLERLEME” İÇİN NET YOL HARİTASI (senin istediğin %100 kapsama yaklaşma)
A) CoinGlass’ta kesin var olanları hemen sisteme ekle (hızlı %70–%85 kapsama)
	•	Daily: Liquidation Map + Heatmap Model2/3 + Orderbook Heatmap + Large Open Orders + Futures vs Spot Volume Ratio + RSI + Spot market volume/volume change.  
	•	Weekly: Exchange Balance + Netflow + Whale Transfer + ETF flows/netassets/premium-discount + Grayscale holdings/premium.  
	•	Monthly: AHR999 + Puell + S2F + Pi Cycle + Golden Ratio + Bubble Index + 2y MA multiplier + RHODL + (gerekli SOPR/supply metrikleri).  

B) CoinGlass’ta belirsiz/isim farklı olabilecekleri Research ile tek tek doğrula (kalan CoinGlass payı)
	•	Premium Index (Coinbase dışında “genel premium”)

	•	MVRV / NVT / Mayer / 4y MA / HashRibbons / PowerLaw / DeltaTop (CoinGlass’ta farklı isimle olabilir; docs menüsünde yoksa endpoint de yoktur, o zaman fallback)  

C) CoinGlass dışı ücretsiz fallback modülleri (kalan %0–%30’u kapatır)
	•	CME/COT: CFTC COT raporu (ücretsiz) → weekly “CME Long/Short”
	•	Miner outflows/revenue: ücretsiz zincir verisi sağlayan kaynaklar (burada Research ile en stabil API’yi seçeceğim)
	•	MVRV/NVT/Mayer/HashRibbons/PowerLaw/DeltaTop: ücretsiz kaynaklar + gerekiyorsa “hesaplayarak türetme” (ör. Mayer = fiyat / 200DMA; NVT = marketcap / tx volume; bazıları tamamen hesapla çözülebilir).

Son olarak: sen “bu araştırmanın her kelimesini hafızaya kaydet” dedin; bende şu an en güvenli yöntem şu: bu mesajı “Project Log – 2026-01-17” diye saklayalım ve repo içine de koyabilelim. Çünkü kalıcı hafıza tarafında, kelimesi kelimesine garanti veremem; ama proje için kritik olan her şeyi yazılı artefakt haline getirirsem kayıp riski sıfır olur. Eğer “OK” dersen Helm Helmer’a tek hamlede şu işi yaptıracağız: repo root’a PROJECT_LOG.md ve SOURCE_MAP.md ekleyip (bu mesajın düzenlenmiş hali), ayrıca registry genişletme için batch3_metrics_system/adapters/ klasör iskeletini açıp “Daily Full / Weekly / Monthly” skeleton’ını commit edeceğiz.

Sen Research’ü aç; ben de hemen sonraki mesajda “Savaş Paneli’nin kalan her metriği için CoinGlass v4’te birebir endpoint adı + URL path + param seti + örnek response shape + normalizer notu” formatında kaynak haritasını çıkartacağım. Bu noktada hedefimiz net: önce CoinGlass ile maksimumu, sonra sadece zorunlu kalanlar için ücretsiz fallback adaptörleri.
